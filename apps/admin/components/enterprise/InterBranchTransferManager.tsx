    ...
    <Card variant="glass" className="p-5 flex flex-col gap-4">
      <div className="flex justify-between items-center pb-2 border-b border-border-subtle">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-text-primary">Inter-Branch Equipment & Asset Transfer Engine</h3>
        </div>
      ...
      <div className="flex flex-col gap-3 text-xs">
        {transfers.map((tr) => (
          <div key={tr.id} className="p-3.5 rounded-xl bg-surface-base border border-border-subtle flex justify-between items-center">
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                <span className="font-bold text-text-primary">{tr.assetName}</span>
                <span className="font-mono text-[10px] text-gold-500 font-semibold">{tr.assetCode}</span>
              </div>
              <span className="text-[10px] text-text-tertiary">Transfer: {tr.fromBranch} ➔ {tr.toBranch}</span>
            </div>
