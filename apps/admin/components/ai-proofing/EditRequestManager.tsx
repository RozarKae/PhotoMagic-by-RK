    ...
    <Card variant="glass" className="p-5 flex flex-col gap-4">
      <div className="flex justify-between items-center pb-2 border-b border-border-subtle">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-text-primary">Client Edit Requests & Retouch Workflow</h3>
        </div>
      ...
      <div className="flex flex-col gap-3 text-xs">
        {requests.map((req) => (
          <div key={req.id} className="p-4 rounded-xl bg-surface-base border border-border-subtle flex flex-col gap-2">
            <div className="flex justify-between items-center">
              <span className="font-bold text-text-primary">{req.photoTitle}</span>
              <div className="flex items-center gap-2">
                <Badge variant={req.priority === 'urgent' || req.priority === 'high' ? 'error' : 'warning'}>
                  {req.priority}
                </Badge>
            ...
            <div className="flex justify-between items-center pt-2 border-t border-border-subtle text-[10px] text-text-tertiary">
              ...
              <div className="flex items-center gap-2">
                <Button variant="ghost" size="sm" onClick={() => handleStatusChange(req.id, 'in_progress')}>
                  In Progress
                </Button>
                <Button variant="ghost" size="sm" onClick={() => handleStatusChange(req.id, 'completed')} className="text-status-success">
                  Complete & Deliver
                </Button>
              </div>
            </div>
