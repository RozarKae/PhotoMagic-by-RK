...
import { FolderPlus, Calendar, HardDrive, ShieldCheck, CheckCircle2, Folder, FileText, Wand2 } from 'lucide-react';
...
export const EventProvisioningManager: React.FC = () => {
  ...
  const folderStructure = [
    { name: '01_RAW_Capture (300 GB)', sub: ['Camera_A_SonyA7IV', 'Camera_B_CanonR5', 'Drone_DJI_Mavic3'] },
    ...
    <Card variant="glass" className="p-6 flex flex-col gap-6">
      <div className="flex justify-between items-center pb-2 border-b border-border-subtle">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-text-primary">Phase 9.1 Automated Event Creation & Folder Provisioning</h3>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 text-xs">
        ...
        <div className="flex flex-col gap-4">
          <div className="p-4 rounded-xl bg-surface-base border border-border-subtle flex flex-col gap-2">
            ...
            <span className="text-[10px] text-text-tertiary font-mono">Date: July 15, 2026 • Allocated: {allocatedStorage} GB R2 Storage</span>
          </div>

          <div className="flex flex-col gap-2">
            <label className="font-bold text-text-primary">Storage Allocation Limit ({allocatedStorage} GB)</label>
            <input
              type="range"
              min={100}
              max={1000}
            ...
            {folderStructure.map((f, i) => (
              <div key={i} className="flex flex-col gap-1 pl-2 border-l border-gold-500/30">
                ...
                <div className="flex flex-wrap gap-2 text-[10px] text-text-tertiary pl-3">
                  {f.sub.map((s) => (
                    <span key={s} className="bg-canvas px-2 py-0.5 rounded border border-border-subtle">
                      {s}
                    </span>
                  ))}
                </div>
