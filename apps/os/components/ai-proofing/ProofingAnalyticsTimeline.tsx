      ...
      <Card variant="glass" className="lg:col-span-2 p-5 flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between gap-2 pt-2 overflow-x-auto">
          {steps.map((s, idx) => (
            <div
              key={idx}
              className="flex flex-col items-center gap-1.5 flex-1 min-w-[90px] text-center"
            >
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs border ${
                  s.completed
                    ? 'bg-gold-500 text-surface-base border-gold-500'
                    : 'bg-surface-base text-text-tertiary border-border-subtle'
                }`}
              >
                {idx + 1}
              </div>
              <span className="font-bold text-[11px] text-text-primary leading-tight">
                {s.title}
              </span>
      ...
      <Card variant="glass" className="p-5 flex flex-col justify-between">
        ...
        <div className="flex flex-col gap-1">
          ...
          <span className="text-xs text-status-success font-semibold">
            High Activity (24 Favorites Marked)
          </span>
        </div>
