...
export const cameraSimulationSchema = z.object({
  ...
  lighting: z.enum(['Golden Hour', 'Softbox', 'Studio', 'Natural', 'Sunset', 'Neon', 'Backlight', 'Rim Light']).default('Golden Hour'),
});

export const generatePhotoBatchSchema = z.object({
  prompt: z.string().min(3, 'Prompt is required'),
  negativePrompt: z.string().optional(),
  provider: z.enum(['openai', 'gemini', 'stability', 'flux', 'ideogram', 'replicate', 'fal']).default('flux'),
